...
import type { PersonnelTeamItem, PersonnelTeamOperationalRow, PersonnelTeamRow } from "./personnel-team-drilldown";

type DrilldownMode = "personnel" | "operational";

const STATUS_SEGMENTS = [
  { key: "missing_unknown", label: "נעדר / לא ידוע", className: "missing" },
  { key: "trapped_located_not_yet_rescued", label: "לכוד שאותר וטרם חולץ", className: "in-progress" },
...
const FIXED_TEAM_ORDER = ["team_1", "team_2", "team_3", "population", "command_post", "logistics", "medical"];
...
function visibleTeams(teams: PersonnelTeamItem[]) {
  return teams
    .filter((team) => FIXED_TEAM_ORDER.includes(team.id) || team.operationalRows.length > 0 || team.total > 0)
    .sort((a, b) => {
      const indexA = FIXED_TEAM_ORDER.indexOf(a.id);
      const indexB = FIXED_TEAM_ORDER.indexOf(b.id);
      if (indexA !== -1 || indexB !== -1) return (indexA === -1 ? 999 : indexA) - (indexB === -1 ? 999 : indexB);
      return a.label.localeCompare(b.label, "he");
    });
}

export function PersonnelTeamCommandWidget({ teams }: { teams: PersonnelTeamItem[] }) {
  const [expanded, setExpanded] = useState(false);
  const [openDrilldown, setOpenDrilldown] = useState<{ teamId: string; mode: DrilldownMode } | null>(null);
  const visible = visibleTeams(teams);
  const chartTeams = visible.filter((team) => team.operationalRows.length > 0);
  const openTeam = visible.find((team) => team.id === openDrilldown?.teamId) ?? null;
  const maxTeamTotal = Math.max(1, ...chartTeams.map((team) => team.operationalRows.length));

  function toggleDrilldown(teamId: string, mode: DrilldownMode) {
    setExpanded(true);
    setOpenDrilldown((current) => (current?.teamId === teamId && current.mode === mode ? null : { teamId, mode }));
  }
    ...
    <div className="personnel-team-dashboard">
            ...
            {chartTeams.length === 0 ? (
              ...
              <div className="team-stacked-chart">
                {chartTeams.map((team) => {
                  ...
                  const height = total === 0 ? 10 : Math.max(18, Math.round((total / maxTeamTotal) * 100));
              ...
              return (
                <article className={`personnel-team-card ${personnelOpen || operationalOpen ? "selected" : ""}`} key={team.id}>
                  ...
                  <dl className="personnel-team-counts">
                    <div><dt>נוכחים</dt><dd>{formatNumber(team.present)}</dd></div>
                    <div><dt>בדרך</dt><dd>{formatNumber(team.enRoute)}</dd></div>
                    <div><dt>לא זמין</dt><dd>{formatNumber(team.unavailable)}</dd></div>
                    <div><dt>לא פעיל</dt><dd>{formatNumber(team.inactive)}</dd></div>
                  </dl>
                  <div className="personnel-team-actions" aria-label={`פעולות ${team.label}`}>
                    <button className={`button compact ${personnelOpen ? "" : "secondary"}`} type="button" aria-expanded={personnelOpen} onClick={() => toggleDrilldown(team.id, "personnel")}>כ״א</button>
                    <button className={`button compact ${operationalOpen ? "" : "secondary"}`} type="button" aria-expanded={operationalOpen} onClick={() => toggleDrilldown(team.id, "operational")}>מספרים מבצעיים</button>
                  </div>
                </article>
              );
            })}
      ...
      {expanded && openTeam && openDrilldown?.mode === "personnel" ? (
        <div className="team-drilldown-panel personnel-dashboard-drilldown">
          <div className="command-section-heading compact-heading"><h3>כ״א - {openTeam.label}</h3></div>
          {openTeam.rows.length === 0 ? (
            <p className="muted">אין אנשי כ״א להצגה בצוות זה.</p>
          ) : (
            <div className="table-scroll">
              <table className="table compact-analysis-table">
                <thead><tr><th>שם מלא</th><th>תפקיד</th><th>טלפון</th><th>סטטוס</th><th>זמן עדכון אחרון</th></tr></thead>
                <tbody>
                  {openTeam.rows.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td>{row.roleLabel}</td>
                      <td>{row.phone ?? "-"}</td>
                      <td><span className={`status-pill ${statusClass(row.attendanceStatus)}`}>{row.attendanceLabel}</span></td>
      ...
      {expanded && openTeam && openDrilldown?.mode === "operational" ? (
        <div className="team-drilldown-panel personnel-dashboard-drilldown">
          <div className="command-section-heading compact-heading"><h3>מספרים מבצעיים - {openTeam.label}</h3></div>
          {openTeam.operationalRows.length === 0 ? (
            <p className="muted">אין מספרים מבצעיים להצגה לצוות זה.</p>
          ) : (
            <div className="table-scroll">
              <table className="table compact-analysis-table">
                <thead><tr><th>מספר מבצעי</th><th>שם</th><th>אתר</th><th>סטטוס אחרון</th><th>תא שטח</th><th>זמן עדכון אחרון</th></tr></thead>
                <tbody>
