...
begin
  ...
  if v_resident.linked_person_id is not null
    and v_resident.linked_person_id <> v_person.id
  then
    raise exception 'Resident is already linked to another operational person';
  end if;
...
comment on function public.link_person_to_resident(uuid, uuid, text)
  is 'Links an existing operational Person to an existing UnitResident, assigns the Person to the resident unit, and writes a new immutable EventLog row.';
