-- Fix resident linking behavior.
--
-- Linking an operational number must:
-- 1. Sync names only when the resident row is empty/placeholder.
-- 2. Map person.first_name to resident.first_name and person.last_name to resident.last_name.
-- 3. Not change resident.status_id just because a link was created.
-- 4. Not mark linked_to_person as gap-resolved/handled.
...
set
  ...
  hebrew_label = 'מקושר למספר מבצעי',
  color = 'orange'
...
comment on function public.link_person_to_resident(uuid, uuid, text)
  is 'Links an operational person to a resident without changing resident status; placeholder resident names are synced from the operational person.';

-- Repair existing linked rows that still clearly look like placeholders or the
-- previously observed swapped/partial bug. Real resident names are left intact.
...
with candidates as (
  ...
  where ur.is_active = true
    ...
    and (
      nullif(btrim(concat_ws(' ', ur.first_name, ur.last_name)), '') is null
      or nullif(btrim(concat_ws(' ', ur.first_name, ur.last_name)), '') ~ '^דייר[[:space:]]+[0-9]+$'
      or (
        nullif(btrim(coalesce(ur.first_name, '')), '') = nullif(btrim(coalesce(p.last_name, '')), '')
        and nullif(btrim(coalesce(ur.last_name, '')), '') ~ '^דייר[[:space:]]+[0-9]+$'
      )
    )
)
update public.unit_residents ur
set
  first_name = nullif(btrim(c.person_first_name), ''),
  last_name = nullif(btrim(c.person_last_name), ''),
  updated_at = now()
from candidates c
where ur.id = c.resident_id;
