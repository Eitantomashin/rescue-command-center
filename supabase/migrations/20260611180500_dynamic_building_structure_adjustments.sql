-- Dynamic building structure adjustments.
-- Forward-only support for adding, splitting and removing apartment units
-- during an active incident without deleting historical unit/resident data.

alter table public.units
  add column if not exists previous_unit_label text,
  add column if not exists original_unit_label text,
  add column if not exists structure_change_type text,
  add column if not exists structure_changed_at timestamptz,
  add column if not exists structure_changed_by uuid references public.profiles(id),
  add column if not exists structure_change_reason text;

create table if not exists public.unit_structure_history (
  ...
  change_type text not null check (
    change_type in ('apartment_added', 'apartment_split', 'apartment_removed', 'apartment_renumbered')
  ),
  previous_label text,
  current_label text,
  reason text,
...
alter table public.unit_structure_history enable row level security;

drop policy if exists unit_structure_history_member_select on public.unit_structure_history;
create policy unit_structure_history_member_select
  on public.unit_structure_history for select
  using (public.can_view_incident(incident_id));

create index if not exists unit_structure_history_unit_id_idx
  on public.unit_structure_history (unit_id, changed_at desc);

create index if not exists unit_structure_history_site_floor_idx
  on public.unit_structure_history (site_id, floor_id, changed_at desc);

create or replace function public.dynamic_structure_unit_label(p_unit public.units)
returns text
language sql
stable
as $$
  select case
    when coalesce(p_unit.zone_type, 'apartment') = 'apartment' then U&'\05D3\05D9\05E8\05D4 ' || p_unit.unit_number
    when p_unit.zone_name is not null then p_unit.zone_name || ' ' || coalesce(p_unit.zone_sequence::text, p_unit.unit_number)
    else U&'\05D9\05D7\05D9\05D3\05D4 ' || p_unit.unit_number
  end
$$;
  ...
  select exists (
    ...
    where ur.unit_id = p_unit_id
      ...
      and (
        ...
        or ur.age is not null
        or nullif(btrim(coalesce(ur.phone, '')), '') is not null
        or nullif(btrim(coalesce(ur.last_name, '')), '') is not null
        or coalesce(ur.gender, 'unknown') <> 'unknown'
        or nullif(btrim(coalesce(ur.notes, '')), '') is distinct from 'placeholder'
        or coalesce(st.status_key, 'missing') <> 'missing'
        or nullif(btrim(coalesce(ur.first_name, '')), '') !~ U&'^\05D3\05D9\05D9\05E8[[:space:]]+[0-9]+$'
      )
  )
$$;
...
create or replace function public.dynamic_structure_record_history(
  p_unit public.units,
  p_change_type text,
  p_previous_label text,
...
as $$
begin
  insert into public.unit_structure_history (
    incident_id,
    site_id,
    floor_id,
    unit_id,
...
begin
  ...
  for v_unit in
    ...
    order by zone_sequence nulls last, created_at
  loop
    v_old_label := coalesce(v_unit.previous_unit_label, U&'\05D3\05D9\05E8\05D4 ' || v_unit.zone_sequence::text, U&'\05D3\05D9\05E8\05D4');
    v_new_label := U&'\05D3\05D9\05E8\05D4 ' || v_new_number;

    update public.units
    set previous_unit_label = coalesce(previous_unit_label, regexp_replace(v_old_label, U&'^\05D3\05D9\05E8\05D4[[:space:]]+', '')),
    ...
    perform public.dynamic_structure_record_history(
      ...
      v_new_label,
      p_reason,
      jsonb_build_object('old_unit_number', v_old_label, 'new_unit_number', v_new_number)
    );
...
begin
  ...
  perform public.dynamic_structure_record_history(
    ...
    p_reason,
    jsonb_build_object('position', v_position, 'default_potential', 5)
  );
  ...
  perform public.create_event_log(
    ...
    U&'\05E0\05D5\05E1\05E4\05D4 ' || public.dynamic_structure_unit_label(v_new_unit) || U&' \05D1\05E7\05D5\05DE\05D4 ' || v_floor.floor_number,
    'operational',
    'important',
    now(),
    v_new_unit.site_id,
    ...
    'ui',
    null,
    jsonb_build_object('unit_id', v_new_unit.id, 'floor_number', v_floor.floor_number, 'current_label', public.dynamic_structure_unit_label(v_new_unit))
  );
...
grant execute on function public.dynamic_structure_unit_label(public.units) to authenticated;
