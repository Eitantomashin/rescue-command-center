-- Phase 6A QA: fix wizard numbering semantics.
--
-- New sites:
-- - Apartments receive continuous apartment numbers across residential floors.
-- - Non-apartment zones receive local zone_sequence per floor and zone type.
-- - unit_number remains an internal unique value per floor for non-apartment zones.
--
-- Existing test sites:
-- - repair_site_wizard_unit_numbering(site_id) can safely renumber units without
--   deleting units or residents.
...
begin
  ...
  perform public.create_event_log(
    ...
    'עודכן מספור דירות ואזורים באתר ' || v_site.site_number,
    'correction',
    'normal',
    now(),
    p_site_id,
...
comment on function public.repair_site_wizard_unit_numbering(uuid)
  is 'Safely repairs Phase 6A wizard unit numbering for an existing site without deleting units or residents.';

-- Validation query after creating or repairing a site:
-- select f.floor_number, u.zone_type, u.unit_number, u.zone_sequence, u.zone_name
-- from public.units u
-- join public.floors f on f.id = u.floor_id
-- where u.site_id = '<site_id>'::uuid
-- order by f.floor_number, u.created_at, u.id;
