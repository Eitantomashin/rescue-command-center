-- Fix Phase 5B resident name sync context.
--
-- The previous function referenced public.resident_event_context, which is not
-- present in deployed databases. This version builds the location context
-- inline from unit/floor data.
...
comment on function public.link_person_to_resident(uuid, uuid, text)
  is 'Links an operational person to a resident, syncing names from operational person to empty/placeholder resident rows with inline location context.';
