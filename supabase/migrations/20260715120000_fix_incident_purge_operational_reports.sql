-- Allow controlled incident purge to remove immutable operational reports.
-- Regular UPDATE/DELETE operations on operational_reports remain blocked.
...
as $$
begin
  if tg_op = 'DELETE' and public.internal_write_allowed('rcc.allow_incident_purge_operational_reports') then
    return old;
  end if;
...
drop trigger if exists operational_reports_immutable on public.operational_reports;
create trigger operational_reports_immutable
  before update or delete on public.operational_reports
  for each row execute function public.prevent_operational_reports_update_or_delete();
...
begin
  ...
  perform set_config('rcc.allow_incident_purge_operational_reports', 'on', true);
  delete from public.operational_reports where incident_id = p_incident_id;
  perform set_config('rcc.allow_incident_purge_operational_reports', 'off', true);
