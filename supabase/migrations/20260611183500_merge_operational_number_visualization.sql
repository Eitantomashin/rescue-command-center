-- Merged operational number visualization and primary-number rules.
-- Only 900-999 numbers are temporary population/intelligence numbers.

create or replace function public.is_temporary_operational_number(p_operational_number integer)
returns boolean
language sql
immutable
as $$
  select p_operational_number between 900 and 999;
$$;
...
begin
  ...
  v_source_is_temporary := public.is_temporary_operational_number(v_source.operational_number);
  v_target_is_temporary := public.is_temporary_operational_number(v_target.operational_number);

  if v_source_is_temporary and not v_target_is_temporary then
    v_primary := v_target;
    v_merged := v_source;
  elsif v_target_is_temporary and not v_source_is_temporary then
    v_primary := v_source;
    v_merged := v_target;
  elsif not v_source_is_temporary and not v_target_is_temporary then
    v_primary := v_source;
    v_merged := v_target;
  elsif v_source.created_at <= v_target.created_at then
    v_primary := v_source;
    v_merged := v_target;
  else
    v_primary := v_target;
  ...
  v_reason := coalesce(
    ...
    '#' || v_merged.operational_number || U&' \05D0\05D5\05D7\05D3 \05E2\05DD #' || v_primary.operational_number
  );
  ...
  perform public.create_event_log(
    ...
    U&'\05D0\05D9\05D7\05D5\05D3 \05DE\05E1\05E4\05E8\05D9\05DD \05DE\05D1\05E6\05E2\05D9\05D9\05DD',
    '#' || v_merged.operational_number || U&' \05D0\05D5\05D7\05D3 \05E2\05DD #' || v_primary.operational_number,
    'merge',
    'important',
    now(),
    ...
    jsonb_build_object(
      ...
      'primary_is_temporary_900_number', public.is_temporary_operational_number(v_primary.operational_number),
      'merged_is_temporary_900_number', public.is_temporary_operational_number(v_merged.operational_number),
...
select
  ...
  coalesce(el.source_type, U&'\05DE\05E2\05E8\05DB\05EA') as information_source_type,
  el.source_name as information_source_name,
  null::text as source_phone,
  null::text as grid_cell,
  U&'\05DC\05D0 \05D9\05D3\05D5\05E2'::text as confidence_level,
...
comment on function public.merge_operational_numbers(uuid, integer, integer, text)
  is 'Merges operational numbers with 900-999-only temporary-number priority, keeps both histories, syncs best known name, and appends immutable EventLog metadata.';
