-- Fix placeholder resident detection during operational-number linking.
--
-- Placeholder names may be stored split as first_name='דייר',
-- last_name='1'. Detect placeholders from the combined display name so both
-- fields are replaced by the linked operational person's name.
...
begin
  ...
  v_resident_has_real_name :=
    v_old_resident_display_name is not null
    and v_old_resident_display_name !~ '^דייר[[:space:]]+[0-9]+$';
...
comment on function public.link_person_to_resident(uuid, uuid, text)
  is 'Links an operational person to a resident and syncs names when the combined resident display name is empty or a placeholder such as דייר 1.';
